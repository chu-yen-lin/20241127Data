...
// 透過 fetch 經由 http://localhost:8084/rest/room 取得遠端資料
const fetchRooms = async () => {
	try {
		const response = await fetch('http://localhost:8084/rest/room');
...
const confirmEdit = async () => {
	try {
		...
		const response = await fetch(`http://localhost:8084/rest/room/${roomId}`, {
			method: 'PUT',
			headers: {
